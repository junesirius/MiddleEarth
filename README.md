...
本项目为[中土相关的粉丝网站](https://junesirius.github.io/MiddleEarth)的源代码，由“[六月的天狼星](https://junesirius.github.io)”搭建、设计、排版、维护。
...
### [\_config.yml](https://github.com/junesirius/MiddleEarth/blob/master/_config.yml)

- name: Middle Earth
- name_en: J.R.R. Tolkien, Christopher Tolkien
- url: 主站链接
- baseurl: 粉丝站根目录
- paginate: 首页的每页文章数量
- permalink: 文章自动生成的网址链接格式

### [\_data/description.yml](https://github.com/junesirius/MiddleEarth/blob/master/_data/description.yml)

个人签名，默认置于头像下方，位置居中。由`<br>`实现换行。

### [feed.xml](https://github.com/junesirius/MiddleEarth/blob/master/feed.xml)

在“[https://junesirius.github.io/MiddleEarth/feed.xml](https://junesirius.github.io/MiddleEarth/feed.xml)”中集成显示所有文章信息。本文档无需改动。

### [LICENSE](https://github.com/junesirius/MiddleEarth/blob/master/LICENSE)
...
### [robots.txt](https://github.com/junesirius/MiddleEarth/blob/master/robots.txt)

面向网络搜索引擎的漫游器（即网络爬虫），设置为拦截所有机器人，禁止爬虫获取本站任意目录。

## 排版布局相关文件

### [\_layouts](https://github.com/junesirius/MiddleEarth/tree/master/_layouts)

本文件夹下的`default.html`和`post.html`确定各文章页的主要排版布局。

#### [default.html](https://github.com/junesirius/MiddleEarth/tree/master/_layouts/default.html)

确定每个采用default布局的页面都包含了页眉（header）、页脚（footer）、侧边栏（sidebar），且侧边栏与主要内容的宽度比例为1:3。

#### [post.html](https://github.com/junesirius/MiddleEarth/tree/master/_layouts/post.html)
...
### [\_includes](https://github.com/junesirius/MiddleEarth/tree/master/_includes)

本文件夹内文件用于设置通用的页眉、页脚、侧边栏样式。

#### [header.html](https://github.com/junesirius/MiddleEarth/tree/master/_includes/header.html)

- 在\<head\>的\<title\>标签下确定各页面标题显示为：“文章标题 | 作者”
- 在\<head\>的\<link rel="icon"\>标签下设置本站的图标，图标存储位置为[favicon.jpg](https://github.com/junesirius/MiddleEarth/blob/master/assets/images/favicon.jpg)
...
#### [footer.html](https://github.com/junesirius/MiddleEarth/tree/master/_includes/footer.html)

- **版权**：所有资源的版权所有者为原作者，时间为当前年份。
- **致谢**：致谢者信息及链接存储于[\_data/thanks.yml](https://github.com/junesirius/MiddleEarth/blob/master/_data/thanks.yml)。

#### [sidebar.html](https://github.com/junesirius/MiddleEarth/tree/master/_includes/sidebar.html)

依次显示：头像、个签、最近更新、热门分类、热门标签

- **头像**：图片与本站图标相同（也可选择不同图片），存储于[favicon.jpg](https://github.com/junesirius/MiddleEarth/blob/master/assets/images/favicon.jpg)，显示为圆形。
- **个签**：文字居中排列，内容存储于[\_data/description.yml](https://github.com/junesirius/MiddleEarth/blob/master/_data/description.yml)。
...
### [\_sass](https://github.com/junesirius/MiddleEarth/tree/master/_sass)

#### [theme.scss](https://github.com/junesirius/MiddleEarth/tree/master/_sass/theme.scss)

包含大部分自定义的css格式，包括页眉、页脚、侧边栏、文章页的主要格式。设置个站背景纹理图片为[canvas_bg.jpg](https://github.com/junesirius/MiddleEarth/blob/master/assets/images/canvas_bg.jpg)。

#### [custom\_color.scss](https://github.com/junesirius/MiddleEarth/tree/master/_sass/custom_color.scss)
...
#### [syntax.scss](https://github.com/junesirius/MiddleEarth/tree/master/_sass/syntax.scss)

来自[Jekyll](https://github.com/jekyll/jekyll)，未作更改。

## 网站主要界面

### [index.html（首页）](https://github.com/junesirius/MiddleEarth/blob/master/index.html)

在`index.html`页面按时间顺序由新到旧显示文章日期、标题、缩略内容。每页文章数由[\_config.yml](https://github.com/junesirius/MiddleEarth/blob/master/_config.yml)中`paginator`后的数值进行调整，本站采用每页7篇文章。

每篇文章的缩略内容由各文章页台头`description`标签后内容确定，若该标签为空，则截取除空格外的前150字符（该数值可调整）。

点击各文章标题可进入文章页。

### [categories.md（分类页）](https://github.com/junesirius/MiddleEarth/blob/master/categories.md)
...
### [categories-detail.md（分类细节）](https://github.com/junesirius/MiddleEarth/blob/master/categories-detail.md)

用于自动生成`categories-detail.html`静态页面，按字母顺序依序排列所有出现过的文章分类（提取自各文章台头的`categories`信息），在每个分类下按时间顺序从新到旧列举所有属于该分类的文章。点击文章标题可进入文章页。

### [tags.md（标签页）](https://github.com/junesirius/MiddleEarth/blob/master/tags.md)

用于自动生成`tags.html`静态页面，按字母顺序依序排列所有出现过的标签（提取自各文章台头的`tags`信息），在每个标签下按时间顺序从新到旧列举所有注有该标签的文章。点击文章标题可进入文章页。

### [characters.md（人物页）](https://github.com/junesirius/MiddleEarth/blob/master/characters.md)

用于自动生成`characters.html`静态页面，按字母顺序依序排列所有出现过的人物（提取自各文章台头的`characters`信息），在每个人物下按时间顺序从新到旧列举所有出现该人物的文章。点击文章标题可进入文章页。

### [pictures.md（图片页）](https://github.com/junesirius/MiddleEarth/blob/master/pictures.md)

用于自动生成`picutures.html`静态页面，按时间顺序排列所有标签中包含“画”和“摄影”的文章中出现过的图片。点击图片可进入文章页。

### [analysis.md（统计页）](https://github.com/junesirius/MiddleEarth/blob/master/analysis.md)
...
### [archive.md（归档页）](https://github.com/junesirius/MiddleEarth/blob/master/archive.md)

用于自动生成`archive.html`静态页面，按时间顺序从新到旧列举所有文章标题，并在右侧标注发表时间。点击文章标题可进入文章页。

### [404.md（报错页）](https://github.com/junesirius/MiddleEarth/blob/master/404.md)
...
### [\_posts](https://github.com/junesirius/MiddleEarth/blob/master/_posts)
...
### [\_drafts](https://github.com/junesirius/MiddleEarth/blob/master/_drafts)

本文件夹存储所有暂未发表在网站的草稿文章，文件标题、台头格式等可不遵守`_posts`的格式要求。当git page网站在线编译时会自动忽略本文件夹，但在本地调试时可通过`--drafts`，命令进行编译查看。

### [assets](https://github.com/junesirius/MiddleEarth/blob/master/assets)
